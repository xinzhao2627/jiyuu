...
export function initDb(): Kysely<DB> {
	const dbPath = join(__dirname, "jiyuuData.db");
	console.log(dbPath);
		...
		},
		{
			...
			up: async () => {
				await db?.schema
					...
					.addColumn("is_muted", "integer", (col) =>
						col.notNull().defaultTo(0)
					)
					...
					.addColumn("restriction_type", "text", (col) =>
						col.defaultTo(null)
					)
					.execute();
		...
		},
		{
			...
			up: async () => {
				await db?.schema
					...
					.addColumn("is_whitelist", "integer", (col) =>
						col.notNull()
					)
					.execute();
		...
		},
		{
			...
			up: async () => {
				try {
					...
					logDbError(
						error,
						"alter-addColumn-block_group-date_created"
					);
				}
			},
			desc: "",
		},
		{
			...
			up: async () => {
				await db?.schema
					...
					.addColumn("date_object", "text", (col) => col.notNull())
					.addColumn("seconds_elapsed", "integer", (col) =>
						col.notNull()
					)
					.execute();
			},
