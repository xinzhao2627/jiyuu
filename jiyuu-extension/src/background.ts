...
import type { Feedback, Feedback_data, TimeListData } from "./interfaces";
	...
	try {
		...
		browser.tabs.query({ active: true }).then(async (tabs) => {
			for (let tab of tabs) {
				...
				if (tab && tab.url && tab.id && /^https?:\/\//.test(tab.url)) {
...
async function reqManipulate(
	activeInfo: browser.Tabs.OnActivatedActiveInfoType | { tabId: number }
) {
